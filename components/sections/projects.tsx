...
import { Github, ExternalLink, Briefcase, Monitor } from 'lucide-react';
...
const project = {
  ...
  githubUrl: 'https://github.com/paramjoshi0702-jpg/lic-website',
  urlSlug: 'lic-website',
};
...
function LaptopMockup() {
  return (
    <div className="relative w-full mx-auto" style={{ maxWidth: 460 }}>
      {/* Screen */}
      <div
        className="relative rounded-t-xl p-2.5 glass-strong border-b-0"
          ...
          <div className="flex items-center gap-1.5 px-3 py-2 bg-black/40 border-b border-white/5">
            ...
            <div className="ml-3 flex-1 h-4 rounded bg-white/5 flex items-center px-2">
              <span className="text-[8px] text-muted-foreground font-mono truncate">
                paramjoshi0702-jpg.github.io/lic-website
              </span>
            </div>
          </div>
          {/* Mock content */}
          <div className={`relative h-full bg-gradient-to-br ${project.gradient} opacity-90`}>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
              ...
              <div className="mt-3 flex gap-1.5 flex-wrap justify-center max-w-[80%]">
                {project.tags.map((t) => (
                  <span key={t} className="text-[8px] px-2 py-0.5 rounded-full bg-white/15 text-white/90 backdrop-blur-sm">
                    {t}
                  </span>
                ))}
              </div>
    ...
    >
      ...
      >
        ...
        <div className="relative z-20">
          ...
          {/* Description */}
          <p className="text-sm md:text-base text-muted-foreground leading-relaxed mb-3">{project.desc}</p>
          <p className="text-sm md:text-base text-foreground/80 italic mb-5">{project.outcome}</p>

          {/* Features */}
          <div className="mb-5">
            <div className="text-xs font-semibold text-gradient-purple uppercase tracking-wider mb-3">
              Key Features
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {project.features.map((f) => (
                <div key={f} className="flex items-center gap-2 text-xs md:text-sm text-muted-foreground">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {project.tags.map((t) => (
              <span
                key={t}
                className="text-xs px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-foreground"
              >
                {t}
              </span>
            ))}
